...
final class DuplicateSetlistControllerTest extends WebTestCase
{
    ...
    {
        return static::getContainer()->get('router')->generate('admin_duplicate_setlist', ['id' => $setlist->getId()]);
    }
    ...
    public function testItRedirectsToEditPageOfNewSetlist(): void
    {
        ...
        static::assertMatchesRegularExpression('#/setlist/\d+/edit#', $location ?? '');
    }

    public function testItCreatesANewSetlistWithSameTitle(): void
    {
        $this->loginAs(MemberRole::Librarian);
        $setlist      = $this->getSetlist(MemberRole::Member);
        $originalTitle = $setlist->getTitle();
        $countBefore  = count($this->em()->getRepository(Setlist::class)->findAll());

        $this->client->request('GET', $this->actionUrl($setlist));

        $this->em()->clear();
        $all = $this->em()->getRepository(Setlist::class)->findAll();
        static::assertCount($countBefore + 1, $all);

        $titles = array_map(fn(Setlist $s) => $s->getTitle(), $all);
        static::assertContains($originalTitle, $titles);
    }
}
